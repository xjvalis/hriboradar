...
const RESEND_FROM = "Rostou? <noreply@rostou.app>";
...
`;
const WRAPPER_END = `
  <p style="color:#8C8A6E;font-size:12px;margin-top:32px">Rostou? · <a href="https://rostou-delta.vercel.app/privacy.html" style="color:#8C8A6E">Zásady ochrany osobních údajů</a></p>
</div>
`;

export function subscriptionActiveEmail(): { subject: string; html: string } {
  return {
    subject: "Vítejte v Rostou? Plus 🍄",
    html: `${WRAPPER_START}
      <h1 style="font-size:20px">Rostou? Plus je aktivní</h1>
...
export function subscriptionCanceledEmail(periodEnd: string | null): { subject: string; html: string } {
  ...
  return {
    subject: "Předplatné Rostou? Plus bylo zrušeno",
    html: `${WRAPPER_START}
      <h1 style="font-size:20px">Mrzí nás, že odcházíte</h1>
      <p>Vaše Rostou? Plus ${untilText}, pak se appka vrátí na bezplatnou verzi. Kdykoli se dá znovu zapnout v appce.</p>
    ${WRAPPER_END}`,
  };
}
...
export function billingIssueEmail(): { subject: string; html: string } {
  return {
    subject: "Problém s platbou za Rostou? Plus",
    html: `${WRAPPER_START}
      <h1 style="font-size:20px">Platba se nezdařila</h1>
      <p>Nepodařilo se nám obnovit vaše předplatné Rostou? Plus - zkontrolujte prosím platební metodu v App Store / Google Play, ať o Plus funkce nepřijdete.</p>
    ${WRAPPER_END}`,
  };
}
