"""
Builds a static forest-composition grid for Czechia from a local OSM PBF
extract (data-prep/osm/czech-republic-latest.osm.pbf, downloaded once from
Geofabrik - not fetched live). Replaces lib/terrain.ts's live per-request
Overpass API calls, which turned out to be unreliable (observed a full
outage of all 3 configured mirrors on 2026-08-27).
...
BOUNDS used by lib/grid.ts (lat 48.55-51.06, lon 12.09-18.87), 2 bytes per
...
0/0 means "no forest data at this cell" - open field, city, water, etc.
...
class ForestAreaCollector(osmium.SimpleHandler):
    """Second-pass handler (fed through AreaManager) - collects every
    forest-tagged assembled area (from a simple way OR a multipolygon
    relation, osmium treats both uniformly as Area objects) as a shapely
    polygon plus its pre-classified (genus_bitmask, leaf_type) pair -
    classifying once per area here, not per grid cell later, is what
    keeps the rasterization pass fast."""

    def __init__(self):
        super().__init__()
        self.wkbfab = osmium.geom.WKBFactory()
        self.polygons = []  # (shapely geometry, genus_mask, leaf_code)
        self.errors = 0

    def area(self, a):
        tags = a.tags
        if tags.get("natural") != "wood" and tags.get("landuse") != "forest":
            return
        try:
        ...
        if genus_mask == 0 and leaf == LEAF_NONE:
            leaf = LEAF_UNKNOWN
        self.polygons.append((geom, genus_mask, leaf))


def collect_forest_areas():
...
def main():
    ...
    print("Assembling forest areas (ways + multipolygon relations)...")
    collector = collect_forest_areas()
    print(f"  {len(collector.polygons):,} forest areas collected, {collector.errors:,} errors ({time.time()-t0:.0f}s)")

    print("Building spatial index...")
    geoms = [p[0] for p in collector.polygons]
    tree = STRtree(geoms)
    prepared = [prep(g) for g in geoms]

    print("Rasterizing...")
    grid = bytearray(N_ROWS * N_COLS * 2)
    filled = 0
    for row in range(N_ROWS):
        lat = LAT_MIN + row * LAT_STEP
        if row % 200 == 0:
            print(f"  row {row}/{N_ROWS} ({time.time()-t0:.0f}s, {filled:,} cells filled so far)")
        for col in range(N_COLS):
            lon = LON_MIN + col * LON_STEP
            cell_center = Point(lon, lat)
            # Query candidates by the cell's bounding box (STRtree over the
            # polygons, not the other way around) - far cheaper than testing
            # every polygon against every cell.
            candidate_idx = tree.query(cell_center)
            if len(candidate_idx) == 0:
                continue
            genus_mask = 0
            leaf = LEAF_NONE
            for idx in candidate_idx:
                if prepared[idx].contains(cell_center):
                    _, gm, lf = collector.polygons[idx]
                    genus_mask |= gm
            ...
            if genus_mask or leaf:
                offset = (row * N_COLS + col) * 2
                grid[offset] = genus_mask & 0xFF
                grid[offset + 1] = leaf
                filled += 1

    pct = filled / (N_ROWS * N_COLS) * 100
    print(f"Done rasterizing: {filled:,} / {N_ROWS*N_COLS:,} cells have forest data ({pct:.1f}%, {time.time()-t0:.0f}s)")
