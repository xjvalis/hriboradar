...
// expo-location has native code - see LocationPickerBody's comment on why
// this needs a guarded require() (harmless on web too, just resolves null).
...
// react-native-webview has no web implementation - same reason
// LocationMapPicker needed a .web.tsx split. This mirrors that: build the
// same pin-picker HTML client-side and drop it in a plain iframe instead
// of fetching it through a WebView. See LocationChangeScreen.tsx (the
// native version) for why this screen exists at all.
export function LocationChangeScreen() {
  ...
  async function useCurrentLocation() {
    ...
    try {
      ...
      if (status !== "granted") {
        setLocateError("Přístup k poloze je zakázaný - povolte ho prohlížeči/telefonu v nastavení.");
        return;
      }
