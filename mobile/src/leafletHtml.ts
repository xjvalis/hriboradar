...
// A minimal single-marker map for "find this place and drop a pin exactly
// where it is" (see LocationMapPicker) - deliberately not buildGridMapHtml
// with an extra mode, since it has none of the grid/legend/species-overlay
// machinery and is meant to load fast on a plain point, not a full-country
// dataset. Shares the same embedded Leaflet build so both pages behave
// identically offline-tile-wise; only the page content differs.
